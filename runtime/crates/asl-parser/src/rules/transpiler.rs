//! ASL VM deterministic code generator and AOT verifier for ASL Rules.
...
impl RulesTranspilerPort for RulesTranspiler {
    fn transpile(
        ...
        let tokens = lexer
            .tokenize()
            .map_err(|e| AslError::RulesTranspileError(format!("Lexical error: {}", e)))?;
        let mut parser = Parser::new(tokens);
        let block = parser
            .parse_rules_block()
            .map_err(|e| AslError::RulesTranspileError(format!("Syntax error: {}", e)))?;

        let mut lines = Vec::new();
        let mut source_map = Vec::new();
        let mut static_invariants = Vec::new();

        // 1. Universal Hermetic ASL VM Preamble
        lines.push(
            "# --- ASL VM DETERMINISTIC PREAMBLE GENERATED BY ASL RULES ENGINE ---".to_string(),
        );
        ...
        // 2. Entrypoint Function Signature
        ...
        source_map.push(SourceMapEntry {
            ...
            description: format!("Entrypoint signature '{}'", entrypoint),
        });

        // 3. Emission of Guard Clauses
        for (i, guard) in block.guards.iter().enumerate() {
            ...
            source_map.push(SourceMapEntry {
                ...
                description: format!("Guard clause for '{}'", guard.target.to_dotted()),
            });
        }
            ...
            if let Some(oth) = &match_sec.otherwise {
                if !is_last_match {
                    return Err(AslError::RulesTranspileError(
                        "'otherwise' clause in intermediate match makes subsequent matches unreachable.".to_string(),
                    ));
                }
                ...
                if let Some(global_oth) = &block.otherwise {
                    ...
                    return Err(AslError::RulesTranspileError(
                        "Exhaustiveness violated: The 'match' block requires an 'otherwise:' clause."
                            .to_string(),
                    ));
                }
            }
        ...
        // If there was no match section, emit global otherwise if present
        ...
        // 5. In-Memory AOT Verification via AstModule
        ...
        AstModule::parse("rules_transpiled.star", starlark_code.clone(), &dialect).map_err(
            |e| {
                AslError::RulesTranspileError(format!(
                    "AOT syntax validation failed for generated Starlark code: {}\nCode:\n{}",
                    e, starlark_code
                ))
            },
        )?;
